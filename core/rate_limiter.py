...
class DistributedRateLimiter:
    ...
    def _fetch_pipeline_data(self, key: str, pipe) -> tuple:
        """Fetch rate limit data from Redis pipeline. Returns (count, backoff, window_start, ttl)."""
    ...
    def _handle_backoff(self, key: str, pipe, backoff: str | None, count: int) -> RateLimitResult | None:
        """Check and handle backoff period. Returns RateLimitResult if backoff active, None otherwise."""
        if not backoff:
            return None
    ...
    def _init_or_reset_window(self, key: str, pipe, window_start, count_raw, window: int) -> tuple[float, int]:
        """Initialize or reset the rate limit window. Returns (window_start, count)."""
        current_time = time.time()
        if not window_start:
            pipe.set(f"{key}:window_start", current_time, ex=window)
        ...
        if current_time - window_start > window:
            ...
            pipe.set(f"{key}:count", 0, ex=window)
            return current_time, 0
        return float(window_start), int(count_raw) if count_raw else 0
    ...
    def _is_allowed_redis(self, key: str, identifier: str, request_limit: Optional[int] = None, request_window: Optional[int] = None) -> RateLimitResult:
        ...
        try:
            ...
            count, backoff, window_start, ttl = self._fetch_pipeline_data(key, pipe)

            # Check if in backoff period
            backoff_result = self._handle_backoff(key, pipe, backoff, count)
            if backoff_result is not None:
                return backoff_result

            # Initialize or reset window
            window_start, count = self._init_or_reset_window(key, pipe, window_start, count, window)
    ...
    def _init_in_memory_entry(self, key: str, now: float) -> dict:
        """Get or create in-memory rate limit entry for the given key."""
        # Get or create entry (thread-safe, caller holds key_lock)
        ...
        return self._in_memory_storage[key]

    def _handle_backoff_case(self, entry: dict, now: float, limit: int) -> Optional[RateLimitResult]:
        """Check and handle backoff period. Returns RateLimitResult if backoff is active."""
        if not entry["backoff_until"] or now >= entry["backoff_until"]:
            return None
    ...
    def _handle_window_reset(self, entry: dict, now: float, window: int) -> None:
        """Reset entry counters if the rate limit window has expired."""
        if now - entry["window_start"] > window:
            entry["count"] = 0
            entry["window_start"] = now
            entry["backoff_until"] = None

    def _handle_limit_exceeded(self, entry: dict, now: float, limit: int) -> Optional[RateLimitResult]:
        """Check if rate limit is exceeded. Returns RateLimitResult if blocked."""
    ...
    def _build_allowed_result(self, entry: dict, now: float, limit: int, window: int) -> RateLimitResult:
        """Build a success RateLimitResult after incrementing the counter."""
        entry["count"] += 1
    ...
    def _is_allowed_in_memory(self, key: str, identifier: str, request_limit: Optional[int] = None, request_window: Optional[int] = None) -> RateLimitResult:
        ...
        with key_lock:
            now = time.time()
            entry = self._init_in_memory_entry(key, now)

            # 1. Check backoff
            backoff_result = self._handle_backoff_case(entry, now, limit)
            if backoff_result is not None:
                return backoff_result

            # 2. Reset window if expired
            self._handle_window_reset(entry, now, window)

            # 3. Check limit
            limit_result = self._handle_limit_exceeded(entry, now, limit)
            if limit_result is not None:
                return limit_result

            # 4. Success — increment and return
            return self._build_allowed_result(entry, now, limit, window)
