...
from core.storage import StorageError, upload_to_ipfs
...
def _validate_vault_ready(config) -> Path | None:
    """Validate vault is armed and encrypted file exists. Returns encrypted_path or None."""
    if not config.armed:
        console.print("[yellow]⚠️  Switch is disarmed - trigger would not fire[/yellow]")
        return None
...
def _display_role_info(config) -> None:
    """Display beneficiary, owner, and vault information."""
...
def _display_vault_info(config, encrypted_path: Path) -> None:
    """Display vault configuration and status."""
    console.print("\n[bold]🔒 Vault:[/bold]")
...
def _display_trigger_status(config) -> None:
    """Show current trigger due status."""
    ...
    if trigger_due:
        console.print(f"   [red]🔴 TRIGGER DUE - {abs(remaining):.1f} days overdue[/red]")
    elif math.isinf(remaining) and remaining > 0:
        console.print("   [yellow]🟡 Never checked in - timer starts on first ping[/yellow]")
    elif remaining > 0:
        console.print(f"   [green]🟢 OK - {remaining:.1f} days remaining[/green]")
    else:
        console.print(f"   [red]🔴 OVERDUE - {abs(remaining):.1f} days overdue[/red]")


def _display_communication_config(config) -> None:
    """Show email and Telegram configuration status."""
    ...
    if email_configured():
        ...
        console.print("      [yellow]Trigger would fail - email delivery unavailable[/yellow]")
...
def _display_delivery_preview(config, encrypted_path: Path) -> None:
    """Show what would be sent in a real trigger."""
    console.print("\n[bold]📦 What would be sent:[/bold]")
    console.print("   1. Email to beneficiary with subject:")
    console.print(f"      '[Lazarus] You have received an inheritance from {config.owner_name}'")
    console.print("   2. Attachments:")
    console.print(f"      • encrypted_secrets.bin ({encrypted_path.stat().st_size:,} bytes)")
    console.print("      • decryption_kit.zip (standalone decrypt.py + instructions)")
...
def test_trigger():
    ...
    try:
        config = load_config()
        encrypted_path = _validate_vault_ready(config)
        ...
        if not pub_key_path.exists():
            ...
            console.print("[yellow]   Trigger would proceed but decryption may fail[/yellow]")

        _display_role_info(config)
        _display_vault_info(config, encrypted_path)
        _display_trigger_status(config)
        _display_communication_config(config)
        _display_delivery_preview(config, encrypted_path)

        console.print("\n[green]✅ Dry run completed successfully[/green]")
        console.print("[yellow]⚠️  This was only a simulation - no actual delivery occurred[/yellow]")
...
# ---------------------------------------------------------------------------
# Update secret helpers
# ---------------------------------------------------------------------------


def _validate_secret_file(new_secret_path: str) -> Path | None:
    """Validate the new secret file exists and is non-empty. Returns Path or None on error."""
    path = Path(new_secret_path)
    if not path.exists():
...
def _load_beneficiary_key(config) -> tuple[Path, object] | None:
    """Load beneficiary public key. Returns (pub_key_path, pub_key) or None on error."""
    pub_key_path = Path(config.beneficiary.public_key_path)
    if not pub_key_path.exists():
        console.print(f"[red]❌ Beneficiary public key not found: {pub_key_path}[/red]")
        return None
    try:
        pub_key = load_public_key_from_file(pub_key_path)
        return pub_key_path, pub_key
    except ValueError as e:
        console.print(f"[red]❌ Invalid beneficiary public key: {e}[/red]")
        return None


def _reencrypt_secret(new_path: Path, beneficiary_pub_key, config) -> tuple[Path, str] | None:
    """Re-encrypt the secret file. Returns (encrypted_path, key_blob) or None on error."""
    output_dir = Path(config.vault.encrypted_file_path).parent
    console.print("[cyan]Re-encrypting file with beneficiary's public key...[/cyan]")
    try:
        return encrypt_file(new_path, beneficiary_pub_key, output_dir)
    except Exception as e:
        console.print(f"[red]❌ Encryption failed: {e}[/red]")
        return None


def _reupload_to_ipfs(encrypted_file_path: Path, config) -> str | None:
    """Re-upload to IPFS if previously configured. Returns new CID or None."""
    if not config.vault.ipfs_cid:
        return None
    console.print("[cyan]Previous IPFS upload detected - re-uploading to IPFS...[/cyan]")
    try:
        cid = upload_to_ipfs(encrypted_file_path)
        console.print(f"[green]✓ Uploaded to IPFS with CID: {cid}[/green]")
        return cid
    except StorageError as e:
        console.print(f"[yellow]⚠️  IPFS upload failed: {e}[/yellow]")
    except Exception as e:
        console.print(f"[yellow]⚠️  Unexpected error during IPFS upload: {e}[/yellow]")
    console.print("[yellow]Continuing with local storage only[/yellow]")
    return None


def _display_update_result(original_path: Path, encrypted_path: Path, new_ipfs_cid: str | None, config) -> None:
    """Display update success message."""
    console.print("[bold green]✓ Secret file updated successfully![/bold green]")
    console.print(f"   Original file: {original_path}")
    console.print(f"   Encrypted file: {encrypted_path}")
    console.print(f"   File size: {encrypted_path.stat().st_size:,} bytes")
    if new_ipfs_cid:
...
def update_secret(new_secret_path: str):
    ...
    try:
        ...
        new_secret_path_obj = _validate_secret_file(new_secret_path)
        if new_secret_path_obj is None:
            return

        key_result = _load_beneficiary_key(config)
        if key_result is None:
            return
        beneficiary_pub_key_path, beneficiary_pub_key = key_result

        encrypt_result = _reencrypt_secret(new_secret_path_obj, beneficiary_pub_key, config)
        if encrypt_result is None:
            return
        encrypted_file_path, key_blob_b64 = encrypt_result

        new_ipfs_cid = _reupload_to_ipfs(encrypted_file_path, config)
        ...
        _display_update_result(new_secret_path_obj, encrypted_file_path, new_ipfs_cid, config)
